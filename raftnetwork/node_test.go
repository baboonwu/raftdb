...
func TestBasicProgress(t *testing.T) {
	...
	leaderNum := waitLeader(nodes)
	log.Println(leaderNum)

	for i := 0; i < 100; i++ {
		s := fmt.Sprintf("somedata_%d", i)
		if err := nodes[0].Propose(context.TODO(), []byte(s)); err != nil {
			log.Println(err)
		}

		time.Sleep(10 * time.Millisecond) // sleep 10 ms
	}
	...
	// check data
	for i := 1; i <= 2; i++ {
		entries := nodes[i].storage.GetAll()
		log.Println(len(entries))
		for i, ent := range entries {
			log.Println(ent.Type, i, string(ent.Data))
		}
	}
